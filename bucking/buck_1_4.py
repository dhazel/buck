...
def buck_1_4(L,log,log_dia,gui_mode):
    ...
    Li = [L,0,0,0,0,0]              #length iterators
                                    ...
                                    #Lengths-to-Check Vector (128 terminated)
    ...
    while s >= 0:
        ...
        if it[s] == 0:                       #eg "top" of tree
            ...
            Li[s] = 0
            Li[s+1] = 0
            while (L - sum(Li)) < (LCV[iterator] + 0.8333):
                if (LCV[iterator] == 128):
                    break
                iterator = iterator + 1

            if (LCV[iterator] == 128) & (s == 0):
...
#        print >>file 
        ...
        file.close()
        os.system("gedit "+sys.path[0]+os.sep+"output.txt &")
